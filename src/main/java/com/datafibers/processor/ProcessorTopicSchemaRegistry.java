...
public class ProcessorTopicSchemaRegistry {
     ...
     */
    public static void forwardGetOneSchema(RoutingContext c, WebClient webClient,
                                           String schemaRegistryRestHost, int schemaRegistryRestPort) {
        final String subject = c.request().getParam("id");
        webClient.get(schemaRegistryRestPort, schemaRegistryRestHost,
                ...
                .send(ar -> {
                    if (!ar.succeeded() || ar.result().statusCode() != ConstantApp.STATUS_CODE_OK) {
						...
						webClient
								.get(schemaRegistryRestPort, schemaRegistryRestHost,
										ConstantApp.SR_REST_URL_CONFIG + "/" + subject)
								.putHeader(ConstantApp.HTTP_HEADER_CONTENT_TYPE, ConstantApp.AVRO_REGISTRY_CONTENT_TYPE)
								.send(arc -> {
									JsonObject res = arc.result().bodyAsJsonObject();
									schema.put(ConstantApp.SCHEMA_REGISTRY_KEY_COMPATIBILITY, !res.containsKey(ConstantApp.SCHEMA_REGISTRY_KEY_COMPATIBILITY_LEVEL) ? "NONE"
											: res.getString(ConstantApp.SCHEMA_REGISTRY_KEY_COMPATIBILITY_LEVEL));
									HelpFunc.responseCorsHandleAddOn(c.response())
											.setStatusCode(ConstantApp.STATUS_CODE_OK).end(Json.encodePrettily(schema));
    ...
    public static void addOneSchemaCommon(RoutingContext c, WebClient webClient,
        ...
        String compatibility = !jsonObj.containsKey(ConstantApp.SCHEMA_REGISTRY_KEY_COMPATIBILITY) ? "NONE"
				: jsonObj.getString(ConstantApp.SCHEMA_REGISTRY_KEY_COMPATIBILITY);
                        ...
                        ar -> {
                            if (!ar.succeeded()) {
								HelpFunc.responseCorsHandleAddOn(c.response())
										...
										.end(DFAPIMessage.getResponseMessage(errorCode, subject, errorMsg + "-SCHEMA"));
    ...
    public static String getCompatibilityOfSubject(String schemaUri, String schemaSubject) {
        String compatibility = null, fullUrl = String.format("http://%s/config/%s", schemaUri, schemaSubject);
