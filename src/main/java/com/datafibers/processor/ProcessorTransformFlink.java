...
public class ProcessorTransformFlink {
     ...
     */
    public static void forwardPostAsSubmitJar(WebClient c, DFJobPOPJ j, MongoClient mongo,
     ...
     */
    public static void forwardDeleteAsCancelJob(RoutingContext c, WebClient webClient,
     ...
     * @param jobID           The job ID to cancel for flink job
     */
    public static void forwardPutAsRestartJob(RoutingContext c, WebClient webClient,
			...
			webClient.delete(flinkRestPort, flinkRestHost, ConstantApp.FLINK_REST_URL + "/" + jobID + "/cancel")
					...
					.sendJsonObject(DFAPIMessage.getResponseJsonObj(1002), ar -> {
						if (!ar.succeeded()) {
							...
							LOG.info(DFAPIMessage.logResponseMessage((ar.result().statusCode() == ConstantApp.STATUS_CODE_OK) ? 1002 : 9012, id));
							forwardPostAsSubmitJar(webClient, j, mongoClient, taskCollection, flinkRestHost,
									flinkRestPort, jarId, allowNonRestoredState, savepointPath, entryClass, parallelism,
									programArgs);
						}
					});
		}
			...
			webClient.get(flinkRestPort, flinkRestHost, ConstantApp.FLINK_REST_URL + "/" + jobId)
					...
					.sendJsonObject(DFAPIMessage.getResponseJsonObj(1003), ar -> {
						if (!ar.succeeded() || ar.result().statusCode() != ConstantApp.STATUS_CODE_OK) {
							HelpFunc.responseCorsHandleAddOn(c.response())
									.setStatusCode(ConstantApp.STATUS_CODE_BAD_REQUEST).end(DFAPIMessage
											.getResponseMessage(9029, taskId, "Cannot Found State for job " + jobId));
